...
const getAllSiteVisitByPid = asyncHandler (async (req, res, next) =>{
    ...
    if(pid != undefined){
        let que = `SELECT site_visit.*, team.name  FROM site_visit JOIN team ON site_visit.visited_by = team.id WHERE pid = ${pid} ORDER BY date DESC;`;
...
const createSiteVisit = async (req, res, next) => {
    upload.single('image')(req, res, function (err) {
        ...
        const { pid, visited_by, notes, date } = req.body;

        if (!pid || !visited_by || !notes || !date) {
            res.status(constants.VALIDATION_ERROR);
        ...
        const insertQuery = `INSERT INTO site_visit (id, pid, visited_by, design_file, notes, date, uploaded_on) VALUES (NULL, '${pid}', '${visited_by}', '${profileUrl}', '${notes}', '${date}', current_timestamp());`;
